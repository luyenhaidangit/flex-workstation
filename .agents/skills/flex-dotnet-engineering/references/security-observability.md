...
Write the message template as a raw dotted business-event key (`domain.subject.event`) — no `[{Class}.{Method}]` bracket prefix. This is the convention already established across the codebase (`instagram.webhook.*`, `instagram.oauth.*`, `meta.auth.*`, `meta.graph.*`); `ILogger<T>` already records the class as the structured `SourceContext` field, and the dotted event key is itself the stable, greppable, aggregatable identity that log tooling (Seq, ELK, Loki...) queries on — an extra `[Class.Method]` prefix would just duplicate `SourceContext` without adding a new dimension. When one message template is shared by several call sites in the same class (e.g. a shared `FetchTokenAsync`/`ExecuteRequestAsync` helper), add an `{Operation}` placeholder to disambiguate instead of branching the event key per call site — see `MetaAuthClient.ExchangeCodeAsync`:

```csharp
logger.LogWarning(
    "meta.auth.provider_error {Operation} {StatusCode} {ReasonPhrase} " +
    "{MetaErrorCode} {MetaErrorSubcode} {MetaErrorType} {MetaTraceId} {ResponseLength}",
    operation, statusCode, response.ReasonPhrase,
    error?.Code, error?.ErrorSubcode, error?.Type, error?.TraceId, body.Length);

// ...

logger.LogError(ex, "meta.auth.request_failed {Operation}", operation);
```
