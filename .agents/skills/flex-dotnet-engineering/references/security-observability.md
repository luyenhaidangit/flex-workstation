...
Write the message template as a raw dotted business-event key (`domain.subject.event`) — no `[{Class}.{Method}]` bracket prefix. Follow whatever dotted-key convention the repository has already established for its own domains; `ILogger<T>` already records the class as the structured `SourceContext` field, and the dotted event key is itself the stable, greppable, aggregatable identity that log tooling (Seq, ELK, Loki...) queries on — an extra `[Class.Method]` prefix would just duplicate `SourceContext` without adding a new dimension. When one message template is shared by several call sites in the same class (e.g. a shared `FetchTokenAsync`/`ExecuteRequestAsync` helper), add an `{Operation}` placeholder to disambiguate instead of branching the event key per call site:

```csharp
logger.LogWarning(
    "payment.gateway.provider_error {Operation} {StatusCode} {ReasonPhrase} " +
    "{ProviderErrorCode} {ProviderErrorType} {ProviderTraceId} {ResponseLength}",
    operation, statusCode, response.ReasonPhrase,
    error?.Code, error?.Type, error?.TraceId, body.Length);

// ...

logger.LogError(ex, "payment.gateway.request_failed {Operation}", operation);
```
