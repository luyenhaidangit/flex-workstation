...
3. **Explain the why.** Even when feedback is terse or frustrated, understand the underlying need and encode *that* understanding, with its rationale, into the instructions. The generic rationale for explaining rules is in SKILL.md's Core Principles — this point is specifically about mining that rationale out of terse feedback, not a restatement.
...
**Claude Code / Cowork (have subagents):** the richer harness becomes available. For each test case you can spawn two subagents in the same turn — one *with* the skill, one *without* (the baseline) — so the comparison is independent. Organize results under a `<skill-name>-workspace/` sibling directory, by iteration (`iteration-1/`, `eval-0/`, ...). You can then grade objective assertions and aggregate a quantitative benchmark (pass rate, time, tokens). This workspace has not ported the eval-viewer script (`eval-viewer/generate_review.py`, from Anthropic's public skill-creator) — `flex-skill-creator`'s `scripts/` holds `sync-skills.ps1` (workspace wiring only), not this one. Skip checking for it and present the raw outputs directly in the conversation for the human to inspect *before* you start critiquing them yourself.
