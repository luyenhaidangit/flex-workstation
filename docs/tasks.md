...
| Done | Cao | Thêm manifest clone repo khi sync workspace | `repos.json` tại project root workstation khai báo repo Flex; `scripts/bootstrap.ps1` clone repo còn thiếu về `C:\Workspace\Project` và không tự `git pull` repo đã tồn tại. |
...
| Done | Cao | Chuyển project root AI tooling vào `flex-workstation` | `OPEN_WORKSPACE.cmd`, `OPEN_CLAUDE.cmd`, `OPEN_CODEX.cmd` và bootstrap dùng `C:\Workspace\Project\flex-workstation` làm project root; template runtime config sync vào repo này. |
