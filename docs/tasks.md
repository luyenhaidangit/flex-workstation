...
| Done | Cao | Hợp nhất entrypoint sync workspace | `SYNC_WORKSPACE.cmd` chạy bootstrap, kiểm tra runtime config Claude/Codex, sync repo project và chuẩn bị local tooling. |
...
| Done | Cao | Đơn giản hóa bootstrap workspace | Bootstrap kiểm tra runtime config `.claude`, `.agents`, `.codex`, `CLAUDE.md` và `AGENTS.md`; không còn sync skill hoặc external source. |
...
| Done | Trung bình | Đồng bộ instruction layer cho Codex | Cập nhật `AGENTS.md` với guardrails tương đương `CLAUDE.md`, dùng thuật ngữ/tooling Codex; bootstrap không còn phụ thuộc `workspaces/templates` đã bị loại khỏi cấu trúc hiện tại. |
