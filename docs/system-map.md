...
`Confirmed`: bootstrap đọc `workstation.json` tại project root workstation và chỉ chạy `git clone` cho repo chưa tồn tại trong `C:\Workspace\Project`. Repo đã có local không bị `pull`, reset hoặc sửa working tree.
...
| --- | --- | --- | --- |
| Workstation config | `workstation.json` | `C:\Workspace\Project\<repo-name>` | Cấu hình workstation; hiện dùng `repositories.items` để clone repo Flex còn thiếu khi chạy sync. |
