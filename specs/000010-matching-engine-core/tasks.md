...
**Kiểm chứng**: Dùng `Flex.Exchange.http`, Swagger và các domain/API tests hiện có. Tests được thêm sau Clean Architecture refactor để bảo vệ các quy tắc matching; chúng không thêm nghiệp vụ ngoài MVP.

> **Lưu ý về path**: Sau khi task list được tạo, service đã refactor thành `Flex.Exchange.Api` → `Flex.Exchange.Application` → `Flex.Exchange.Domain` ← `Flex.Exchange.Infrastructure`. Các task bên dưới được đánh dấu theo trách nhiệm tương ứng trong cấu trúc này; path lịch sử được giữ lại để bảo toàn traceability với plan ban đầu.
...
- [X] T007 [P] Tạo enum `OrderSide`, `OrderStatus` và `RejectReason` theo data model trong `flex-exchange-service/src/Flex.Domain/Enums/{OrderSide.cs,OrderStatus.cs,RejectReason.cs}`
- [X] T008 [P] Tạo command `PlaceOrder` và `CancelOrder` mang `BrokerId` trong `flex-exchange-service/src/Flex.Domain/Commands/{PlaceOrder.cs,CancelOrder.cs}`
- [X] T009 [P] Tạo entity cấu hình và lệnh với giá/khối lượng `long`, `SequenceNumber` và chuyển trạng thái trong `flex-exchange-service/src/Flex.Domain/Entities/{InstrumentConfig.cs,Order.cs}`
- [X] T010 [P] Tạo entity `Trade`, `OrderBookSnapshot` và các giá-level snapshot trong `flex-exchange-service/src/Flex.Domain/Entities/{Trade.cs,OrderBookSnapshot.cs}`
- [X] T011 [P] Tạo base event và bốn event contract có `EventSequence`/`BrokerId` trong `flex-exchange-service/src/Flex.Domain/Events/{ExchangeEvent.cs,OrderAccepted.cs,OrderRejected.cs,TradeExecuted.cs,OrderCancelled.cs}`
- [X] T012 Tạo result type cho place/cancel command theo contract trong `flex-exchange-service/src/Flex.Domain/Matching/{PlaceOrderResult.cs,CancelOrderResult.cs}`
- [X] T013 Tạo cấu trúc order book hai phía với API thêm/gỡ/duyệt theo giá và FIFO xác định trong `flex-exchange-service/src/Flex.Domain/Entities/OrderBook.cs`
- [X] T014 [P] Tạo API response contracts trong `flex-exchange-service/src/Flex.Exchange.Api/Models/{PlaceOrderResponse.cs,CancelOrderResponse.cs}`
- [X] T015 [P] Tạo global exception handler trong `flex-exchange-service/src/Flex.Exchange.Api/ExceptionHandling/GlobalExceptionHandler.cs`
- [X] T016 [P] Tạo cấu hình Serilog, correlation ID và request/response logging middleware trong `flex-exchange-service/src/Flex.Exchange.Api/Observability/{CorrelationIdMiddleware.cs,GlobalLoggingMiddleware.cs}`
- [X] T017 [P] Tạo cấu hình Swagger và JSON dùng chung trong `flex-exchange-service/src/Flex.Exchange.Api/{OpenApi/SwaggerConfiguration.cs,Json/JsonOptions.cs}`
- [X] T018 Tạo `ExchangeOptions`, bind `Exchange:Instrument` và chuyển sang `InstrumentConfig` trong `flex-exchange-service/src/Flex.Exchange.Infrastructure/Configuration/ExchangeOptions.cs`
- [X] T019 Tạo cấu hình FXS (20.000, tick 100, trần 21.400, sàn 18.600, lô 100) và Serilog trong `flex-exchange-service/src/Flex.Exchange.Api/{appsettings.json,appsettings.Development.json}`
- [X] T020 Tạo bootstrap host, đăng ký middleware/Swagger và DI options trong `flex-exchange-service/src/Flex.Exchange.Api/{Program.cs,Extensions/ApplicationExtensions.cs}`
...
- [X] T022 [US1] Bổ sung snapshot read-only rỗng/đầy đủ cùng event log append-only trong `flex-exchange-service/src/Flex.Domain/Matching/MatchingEngine.cs` (phụ thuộc T021)
- [X] T023 [P] [US1] Tạo DTO request/response đặt lệnh map đúng contract trong `flex-exchange-service/src/Flex.Exchange/Models/{PlaceOrderRequest.cs,PlaceOrderResponse.cs}`
- [X] T024 [US1] Tạo `IExchangeService` và `ExchangeService` singleton bọc engine, serialize place/read bằng lock và ghi log nghiệp vụ trong `flex-exchange-service/src/Flex.Exchange/Services/{Interfaces/IExchangeService.cs,ExchangeService.cs}` (phụ thuộc T018, T021, T022)
- [X] T025 [US1] Implement `POST /api/orders` với business reject HTTP 200 qua response wrapper trong `flex-exchange-service/src/Flex.Exchange/Controllers/OrdersController.cs` (phụ thuộc T023, T024)
- [X] T026 [US1] Implement `GET /api/orderbook` và `GET /api/events` theo REST contract trong `flex-exchange-service/src/Flex.Exchange/Controllers/{OrderBookController.cs,EventsController.cs}` (phụ thuộc T024)
- [X] T027 [US1] Thêm kịch bản không khớp, khớp toàn phần và truy vấn snapshot/event vào `flex-exchange-service/src/Flex.Exchange/Flex.Exchange.http` (phụ thuộc T025, T026)
...
- [X] T029 [US2] Hoàn thiện tổng hợp price level và danh sách lệnh FIFO trong snapshot trong `flex-exchange-service/src/Flex.Domain/Entities/OrderBook.cs` (phụ thuộc T013, T028)
- [X] T030 [US2] Thêm kịch bản khớp một phần và kiểm tra `GET /api/orderbook` vào `flex-exchange-service/src/Flex.Exchange/Flex.Exchange.http` (phụ thuộc T028, T029)
...
- [X] T031 [US3] Sắp xếp bid giảm/ask tăng và FIFO bằng `SequenceNumber`, bảo đảm không dùng `ReceivedAt` trong priority tại `flex-exchange-service/src/Flex.Domain/Entities/OrderBook.cs` (phụ thuộc T029)
...
- [X] T033 [US3] Thêm kịch bản ưu tiên giá, FIFO và khớp xuyên nhiều mức vào `flex-exchange-service/src/Flex.Exchange/Flex.Exchange.http` (phụ thuộc T032)
...
- [X] T035 [P] [US4] Tạo `CancelOrderResponse` theo contract trong `flex-exchange-service/src/Flex.Exchange/Models/CancelOrderResponse.cs`
- [X] T036 [US4] Mở rộng `IExchangeService`/`ExchangeService` với cancel được serialize bằng cùng lock và log kết quả trong `flex-exchange-service/src/Flex.Exchange/Services/{Interfaces/IExchangeService.cs,ExchangeService.cs}` (phụ thuộc T034)
- [X] T037 [US4] Implement `DELETE /api/orders/{orderId}` nhận `brokerId` query string và trả business reject HTTP 200 trong `flex-exchange-service/src/Flex.Exchange/Controllers/OrdersController.cs` (phụ thuộc T035, T036)
- [X] T038 [US4] Thêm kịch bản hủy thành công, hủy lặp, hủy sau filled và `brokerId` rỗng vào `flex-exchange-service/src/Flex.Exchange/Flex.Exchange.http` (phụ thuộc T037)
...
- [X] T039 [US5] Implement `OrderValidator` kiểm tra BrokerId, symbol, price, tick, band, quantity và lot theo `InstrumentConfig` trong `flex-exchange-service/src/Flex.Domain/Matching/OrderValidator.cs` (phụ thuộc T007–T009)
- [X] T040 [US5] Gọi validator trước cấp order sequence/mutate sổ, phát `OrderRejected` không có `OrderId` và trả reason trong `flex-exchange-service/src/Flex.Domain/Matching/MatchingEngine.cs` (phụ thuộc T039)
- [X] T041 [US5] Thêm kịch bản validate từng `RejectReason` và xác nhận snapshot bất biến vào `flex-exchange-service/src/Flex.Exchange/Flex.Exchange.http` (phụ thuộc T040)
...
- [X] T042 Kiểm tra và loại mọi dependency ASP.NET/package runtime khỏi domain project trong `flex-exchange-service/src/Flex.Domain/Flex.Domain.csproj`
- [X] T043 Kiểm tra `ExchangeService` serialize toàn bộ place/cancel/snapshot/event read bằng một lock, không để trạng thái nửa command, trong `flex-exchange-service/src/Flex.Exchange/Services/ExchangeService.cs`
- [X] T044 Kiểm tra engine không dùng clock/random/hash iteration cho ID, event sequence hoặc priority trong `flex-exchange-service/src/Flex.Domain/Matching/{MatchingEngine.cs,OrderValidator.cs}`
- [X] T045 Kiểm tra launch profile chỉ dùng cho local demo và không thêm auth/secret vào config trong `flex-exchange-service/src/Flex.Exchange/{Properties/launchSettings.json,appsettings.json,appsettings.Development.json}`
- [X] T046 Cập nhật README với restore/build/run, Swagger và quy trình demo `.http` trong `flex-exchange-service/README.md`
- [X] T047 Hoàn thiện tám nhóm kịch bản manual và hướng dẫn so sánh determinism sau restart trong `flex-exchange-service/src/Flex.Exchange/Flex.Exchange.http`
- [X] T048 Chạy `dotnet restore Flex.Exchange.sln`, `dotnet build Flex.Exchange.sln` và toàn bộ kịch bản trong `specs/000010-matching-engine-core/quickstart.md`; xác nhận 8 nhóm `.http` hoàn tất trong ≤ 5 giây không tính khởi động Kestrel, rồi review commands, bốn events và snapshot trong `specs/000010-matching-engine-core/contracts/exchange-core.md` đối chiếu implementation/Swagger cho MVP 02
