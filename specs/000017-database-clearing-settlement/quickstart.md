...
2. Chạy `liquibase validate` và `update-sql` cho từng master changelog; CI/CD hoặc Kubernetes Job chạy `update` theo thứ tự `exchange` → `broker` → `vsd`. Seed Alpha/Beta chỉ dùng local/test.
