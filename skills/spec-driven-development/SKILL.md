...
description: >
  ...
  Done when a reviewed spec, implementation plan, and task breakdown are approved or explicitly handed off.
---
...
Execute tasks one at a time. Load skill `incremental-implementation` and `test-driven-development` if available. Use skill `context-engineering` if available to load the right spec sections and source files at each step rather than flooding the agent with the entire spec.
